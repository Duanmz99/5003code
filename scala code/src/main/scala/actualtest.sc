...
val rdd = sc.textFile("/Users/duanmz/untitled2/src/main/scala/new_test.txt")
val mapRDD= rdd.map(line=>Row(line.split(" ")(0)))
val sf1=new StructField("id",StringType,true) //这里是列1   信息是 IP
//val sf2=new StructField("user",StringType,true) //这里是列2  信息是user

val table_sch=new StructType(Array(sf1)) //生成表结构 , 由两列叫IP和user的列组成的表 ,可以为空
...
df.select(
  ...
  map(
    Seq(0.005, 0.01, 0.008, 0.009).flatMap { error =>
